    ...
    \item The most important rule to evaluate a model is to avoid biased results
        ...
        \begin{enumerate}
            ...
            \item Take the second part of my dataset $\mathcal{S}_2(|\mathcal{S}_2| = N)$
                ...
                h_{i^*} &\in \mathcal{H} \text{ with } i^* = \argmin_{i} r_i &
                \end{aligned}
                $$
...
A function to calculate the mean squared error is also define to verify the results
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{regression/plots/linear_regression.png}
...
\begin{lstlisting}
y_ln = np.log(y)
w_ln, b_ln, h_ln = linear_regression(X, y_ln)
y_hat_exp = np.exp(h_ln(X))
...
$$
\argmin_{a,b} \sum_{i=1}^{N} (y_i - h(x_i))^2 \;\neq\; \argmin_{a,b} \sum_{i=1}^{N}
(\ln y_i - (a x_i + b))^2,
$$
where $h(x) = \exp(ax+b)$.

Lets consider a data set with two points, $S = \{(x_1, y_1), (x_2, y_2)\}$, where:
$$
\begin{aligned}
& x_1 = x_2 = 0 \\
& y_1 = e^{-1}, y_2 = e^{1}.
...
Since $x_1 = x_2 = 0$, the model $h'(x) = ax + b$ evaluates to the same value
regardless of $a$. So in both fitting problems we are choosing a single constant
prediction of $b$ for the two points.

First we minimize on the original scale
$$
\begin{aligned}
    \sum_{i=1}^{N} (y_i - h(x_i))^2 &= (y_1 - exp(b))^2 + (y_2 - exp(b))^2 \\
    \frac{d}{db} &= -2(y_1 - \exp(b)) - 2(y_2 - \exp(b)) = 0 \\
        \implies & \exp(b) = \frac{e^{-1} + e^{1}}{2} \approx 1.543
\end{aligned}
$$

Then we minimize on the logarithmic scale
$$
\begin{aligned}
    \sum_{i=1}^{N} (\ln y_i - (a x_i + b))^2 &= (\ln y_1 - b)^2 + (\ln y_2 - b)^2 \\
    \frac{d}{db} &= -2(\ln y_1 - b) - 2(\ln y_2 - b) = 0 \\
        \implies & b = \frac{-1 + 1}{2} = 0
\end{aligned}
$$

\paragraph{Conclusion.} The optimal constant fit on the original scale is
$b \approx 1.543$, while the optimal constant fit obtained via the log-transformed
objective is $h(0) = 1$. Since $1.543 \neq 1$, the parameters minimizing
$\sum_i (y_i - h(x_i))^2$ differ from those minimizing $\sum_i (\ln y_i - (ax_i+b))^2$.

\subsection{Task 4 (5 points)}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{regression/plots/non_linear_exp_regression.png}
    \caption{Non-linear $h(x) = exp(ax + b)$ regression}
    \label{fig:non_linear_exp_regression}
\end{figure}
...
The coefficient of determination of the non-linear model $h(x) = exp(ax+b)$ is $R^2 = 0.357$.
...
y_ln = np.log(y)
X_sqrt = np.sqrt(X)
w_sqrt_ln, b_sqrt_ln, h_sqrt_ln = linear_regression(X_sqrt, y_ln)
y_hat_sqrt_exp = np.exp(h_sqrt_ln(X_sqrt))
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{regression/plots/non_linear_sqrt_exp_regression.png}
    \caption{Non-linear $h(x) = exp(a\sqrt{x} + b)$ regression}
    \label{fig:non_linear_sqrt_exp_regression}
\end{figure}

The coefficient of determination of the non-linear model $h(x) = exp(a\sqrt{x}+b)$ is $R^2 = 0.482$.
The improved $R^2$ score demonstrates that the underlying relationship is closer
to a square-root dependency than a strictly linear one. Because this new feature
space captures the data's trajectory more accurately, the resulting model fits
the data better, reducing the mean squared error.
